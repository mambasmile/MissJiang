...
all_data_file = unicode(r'../input/lexin_train02/训练数据7.0.csv', 'utf-8')
...
test_data_file = unicode(r'../input/lexin_test/验证数据集_4/测试数据7.0.csv', 'utf-8')
...
feature_imp_path = unicode(r'../input/分类特征权重0.csv', 'utf-8')
features = []
...
# GradientBoostingClassifier(learning_rate=0.1, n_estimators=140, max_depth=3,
#                                    min_samples_split=1000, min_samples_leaf=70,
#                                    subsample=0.9, random_state=10, max_features=20),
'''模型融合中使用到的各个单模型'''
from sklearn.ensemble import AdaBoostClassifier
clfs = [
    RandomForestClassifier(n_estimators=140,max_depth=11,min_samples_leaf=15, n_jobs=-1, criterion='entropy'),
        ...
        XGBClassifier(learning_rate=0.1, n_estimators=120, max_depth=4, min_child_weight=6,
                      gamma=0, subsample=0.6, colsample_bytree=0.9, objective='binary:logistic',
                      nthread=4, scale_pos_weight=1, seed=15)
        ]
...
# X, X_predict, y, y_predict = train_test_split(preprocessing.normalize(all_data[0:50000][features].values), train_data['dep'].values, test_size=0.33, random_state=2017)
